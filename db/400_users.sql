create function app_public.current_user_id() returns varchar as $$
  select nullif(current_setting('jwt.claims.user_id', true), '')::varchar;
...
create table app_public.users (
  id varchar not null primary key default (public.gen_random_uuid())::varchar,
...
grant select on app_public.users to appainter_visitor;
grant update(name, avatar_url) on app_public.users to appainter_visitor;
grant delete on app_public.users to appainter_visitor;
...
create table app_private.user_secrets (
  user_id varchar not null primary key references app_public.users,
  password_hash text,
...
create table app_private.user_emails (
  ...
  user_id varchar not null default app_public.current_user_id() references app_public.users on delete cascade,
...
create unique index uniq_user_emails_verified_email on app_private.user_emails(email) where is_verified is true;
alter table app_private.user_emails enable row level security;
create trigger _100_timestamps
  before insert or update on app_private.user_emails
  for each row
  execute procedure app_private.tg__update_timestamps();
-- create trigger _900_send_verification_email
--   after insert on app_private.user_emails
...
-- frontend teams.
comment on table app_private.user_emails is
  E'@omit all\nInformation about a user''s email address.';
comment on column app_private.user_emails.email is
  E'The users email address, in `a@b.c` format.';
comment on column app_private.user_emails.is_verified is
...
create policy select_own on app_private.user_emails for select using (user_id = app_public.current_user_id());
create policy insert_own on app_private.user_emails for insert with check (user_id = app_public.current_user_id());
create policy delete_own on app_private.user_emails for delete using (user_id = app_public.current_user_id());
-- grant select on app_private.user_emails to appainter_visitor;
-- grant insert (email) on app_private.user_emails to appainter_visitor;
-- grant delete on app_private.user_emails to appainter_visitor;

--------------------------------------------------------------------------------

create table app_private.user_email_secrets (
  user_email_id int primary key references app_private.user_emails on delete cascade,
...
create table app_public.user_authentications (
  id serial primary key,
  user_id varchar not null references app_public.users on delete cascade,
...
grant select on app_public.user_authentications to appainter_visitor;
grant delete on app_public.user_authentications to appainter_visitor;
...
create function app_private.forgot_password(email text) returns boolean as $$
declare
  v_user_email app_private.user_emails;
  v_reset_token text;
...
create function app_private.reset_password(user_id varchar, reset_token text, new_password text) returns app_public.users as $$
declare
  v_user app_public.users;
  v_user_secret app_private.user_secrets;
  v_reset_max_duration interval = interval '3 days';
...
comment on function app_private.reset_password(user_id varchar, reset_token text, new_password text) is
...
begin
  ...
  if email is not null then
    insert into app_private.user_emails (user_id, email, is_verified)
...
begin
  ...
  if v_matched_authentication_id is null then
    if f_user_id is not null then
      ...
      select * into v_user_email from app_private.user_emails where email = v_email and is_verified is true;
      if not (v_user_email is null) then
        -- User exists!
        insert into app_public.user_authentications (user_id, service, identifier, details) values
